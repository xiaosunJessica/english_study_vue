...
import { type Word, type TestPaper, type Chapter } from "@/stores/vocabulary";
...
onMounted(async () => {
  try {
    ...
    chapters.value = res.data?.list?.map((item: Chapter) => ({
      ...item,
      subtitle: item.intro,
      testPapers: item.list.map((paper: TestPaper) => ({
        ...paper,
